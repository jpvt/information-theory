...
// Note: This implementation uses the BST version of LZW from https://github.com/nwin/lzw/blob/master/src/lzw.rs
...
mod test {
    use super::*;
    use bitstream::{LsbWriter, LsbReader, MsbWriter, MsbReader};
    ...
    #[test]
    fn test_msb_round_trip() {        
        let size = 8;
        let data = b"TOBEORNOTTOBEORTOBEORNOT";
        let mut compressed = vec![];
        {
            let mut enc = Encoder::new(MsbWriter::new(&mut compressed), size).unwrap();
            enc.encode_bytes(data).unwrap();
        }
        println!("{:?}", compressed);
        let mut dec = Decoder::new(MsbReader::new(), size);
        let mut compressed = &compressed[..];
        let mut data2 = vec![];
        while compressed.len() > 0 {
            let (start, bytes) = dec.decode_bytes(&compressed).unwrap();
    ...
    fn test_corpus_round_trip() {
        // ENCODING
        let corpus = std::fs::read_to_string("data/corpus.txt").expect("Should have been able to read the file");
        ...
        std::fs::write("data/encoded_corpus.txt", &compressed).expect("Should have been able to write the file");
        ...
        std::fs::write("data/decoded_corpus.txt", &data2).expect("Should have been able to write the file");
        assert_eq!(data2, data)
    }
}